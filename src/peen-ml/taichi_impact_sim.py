...
try:
    ...
    class ShotPeenParams:  # type: ignore
        ...
        @property
        def R(self) -> float: return self.D / 2.0
        @property
        def Ms(self) -> float:
            return (4.0 / 3.0) * math.pi * self.R ** 3 * self.rho_s
        @property
        def Vn(self) -> float: return self.V * math.sin(self.phi)
...
class MPMShotPeenSolver:
    ...
    def __init__(
        ...
        if not _TAICHI_AVAILABLE:
            raise ImportError(
                "taichi is required for MPM simulation.\n"
    ...
    def _p2g(self):
        ...
        for p in self.x:
            ...
            affine = -4.0 * float(self.dt) * float(self.p_vol) * inv_dx * inv_dx * stress_p + \
    ...
    def _grid_ops(
        self,
        shot_cx: float, shot_cy: float, shot_cz: float,
        shot_vx: float, shot_vy: float, shot_vz: float,
        shot_r:  float,
    ...
    def run(self, n_steps: int = 600, record_every: int = 20):
        ...
        for step in range(n_steps):
            ...
            self._grid_ops(
                ...
                imp_x_field, imp_y_field, imp_z_field,
            )
            self._g2p()
            ...
            if step % record_every == 0:
                ...
                if (impact_occurred
                        and self.shot_vel[2] > 0
                        and self.shot_center[2] > self.shot_radius * 2.0):
                    if self.verbose:
            ...
            if self.verbose and step % 100 == 0:
                ...
                print(f"  step {step:4d}/{n_steps}  t={t*1e9:.1f} ns  "
    ...
    def extract_results(
        ...
        col_mask = (
            np.sqrt((x_phys[:, 0] - ic_x) ** 2 + (x_phys[:, 1] - ic_y) ** 2)
            < col_radius
        )
        ...
        if save_npy:
            ...
            np.save(os.path.join(output_dir, "energy_history.npy"),
                    np.stack([np.array(self.time_hist),
                               np.array(self.ke_target_hist),
                               np.array(self.ke_shot_hist)], axis=1) if self.time_hist else np.zeros((0, 3)))
...
def compare_results(
    ...
    if "sR_depth_profile" in mpm and len(mpm["sR_depth_profile"]) > 0:
        ...
        ax.plot(sR_m[:, 0] * 1e6, sR_m[:, 1] / 1e6,
                label="MPM", color="steelblue", linewidth=1.5)

    if "stress_field" in analytical and "Z" in analytical["stress_field"]:
        sf = analytical["stress_field"]
        ax.plot(sf["Z"] * 1e6, sf["sR"] / 1e6,
                label="Shen & Atluri (analytical)", color="firebrick",
    ...
    if "eps_depth_profile" in mpm and len(mpm["eps_depth_profile"]) > 0:
        ...
        ax.plot(ep_m[:, 0] * 1e6, ep_m[:, 1],
                label="MPM ε_p", color="steelblue", linewidth=1.5)

    if "stress_field" in analytical:
        sf = analytical["stress_field"]
        ax.plot(sf["Z"] * 1e6, sf["eps_avg"],
                label="Analytical ε̄p", color="firebrick",
                linewidth=1.5, linestyle="--")
    ...
    if len(mpm.get("time_hist", [])) > 0:
        ...
        ax.axhline(0, color="k", linewidth=0.4, linestyle="--")
        ax.fill_between(t, vz, 0, where=(np.array(vz) < 0),
                        alpha=0.15, color="firebrick", label="Approach phase")
        ax.fill_between(t, vz, 0, where=(np.array(vz) > 0),
                        alpha=0.15, color="steelblue", label="Rebound phase")
    ...
    fig.suptitle("MPM Numerical  vs  Shen & Atluri (2006) Analytical",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="MPM shot-peen impact simulation (Taichi backend)."
    )
    ...
    parser.add_argument("--compare", action="store_true",
                        help="Also run Shen & Atluri analytical and plot comparison")
    ...
    if args.plot:
        solver.plot_energy_history(
            save_path=os.path.join(args.output, "energy_history.png")
        )
    ...
    if args.compare:
        try:
            analytical = run_analytical_simulation(
                ...
                Nx=20, Ny=20, verbose=False,
            )
            compare_results(
                mpm_results, analytical,
