...
for sample_idx in range(num_simulations):
    ...
    expansion_data = [(0.005, "005"), (0.01, "01"), (0.015, "015"), (0.02, "02")]
    ...
    for coef, name_suffix in expansion_data:
        ...
        mdb.models["Script"].HomogeneousShellSection(
            name=section_name,
            preIntegrate=OFF,
    ...
    expansion_coefficients = {"Peen-005": 0.005, "Peen-01": 0.01, "Peen-015": 0.015, "Peen-02": 0.02}
    ...
    mdb.models["Script"].DisplacementBC(
        name="BC-Fixed",
        createStepName="Initial",
        region=regionToolset.Region(vertices=bottom_left_vertex),
        u1=SET,
        u2=SET,
        u3=SET,
    )
    ...
    mdb.models["Script"].DisplacementBC(
        name="BC-Roll-TopLeft",
        createStepName="Initial",
        region=regionToolset.Region(vertices=top_left_vertex),
        u1=SET,
        u2=UNSET,
        u3=UNSET,
    )
    ...
    mdb.models["Script"].DisplacementBC(
        name="BC-Roll-BottomRight",
        createStepName="Initial",
        region=regionToolset.Region(vertices=bottom_right_vertex),
        u1=UNSET,
        u2=SET,
        ur3=UNSET,
    )
    ...
    np.savetxt(
        os.path.join(simulation_folder, "stress_element_labels.csv"), stress_element_labels, fmt="%d", delimiter=","
    )
    ...
    np.savetxt(
        os.path.join(simulation_folder, "element_connectivity.csv"), element_connectivity, fmt="%d", delimiter=","
    )
