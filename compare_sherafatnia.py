...
def compare(
    ...
    Z_sh_mm = sh["Z"] * 1e3
    Z_sa_mm = sa["Z"] * 1e3
    ax.plot(Z_sh_mm, sh["sigma_x"] / 1e6, label=r"Sherafatnia $\sigma_x$ (loading)", color="tab:blue", lw=1.5)
    ax.plot(
        Z_sh_mm,
        sh["sigma_z"] / 1e6,
        label=r"Sherafatnia $\sigma_z$ (loading)",
        color="tab:blue",
        lw=1.5,
        linestyle="--",
    )
    ax.plot(Z_sa_mm, sa["sigma_xe"] / 1e6, label=r"Shen & Atluri $\sigma_x^e$ (loading)", color="tab:orange", lw=1.5)
    ax.plot(
        Z_sa_mm,
        sa["sigma_ze"] / 1e6,
        label=r"Shen & Atluri $\sigma_z^e$ (loading)",
        color="tab:orange",
        lw=1.5,
        linestyle="--",
    )
    ...
    ax = axes[1]
    ax.plot(Z_sa_mm, sa["sR"] / 1e6, label=r"Shen & Atluri $\sigma_R$ (post-impact residual)", color="tab:red", lw=2)
    ax.axhline(0, color="k", lw=0.5, linestyle="--")
    ax.axhline(-sy / 1e6, color="gray", lw=0.8, linestyle=":", label=f"$σ_y$={sy/1e6:.0f} MPa")
    ax.fill_between(
        Z_sa_mm, sa["sR"] / 1e6, 0, where=sa["sR"] < 0, alpha=0.15, color="tab:red", label="Compressive zone"
    )
    ax.set_xlabel("Depth from surface (mm)")
    ax.set_ylabel("Residual Stress (MPa)")
    ax.set_title(
        "Post-impact residual stress — Shen & Atluri\n" "(Sherafatnia has no unloading → no residual stress estimate)"
    )
    ...
    fig.suptitle(
        f"Sherafatnia formula vs Shen & Atluri (2006) | " f"D={D*1e3:.1f}mm  V={V:.0f}m/s  σ_y={sy/1e6:.0f}MPa",
        fontsize=10,
        y=1.01,
    )
    fig.tight_layout()
