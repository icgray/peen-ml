...
def make_pred_vs_gt():
    ...
    MODEL  = os.path.join(DATA, "saved_model_conv",
    ...
    smape    = float(np.mean(2*np.abs(error) /
                             (np.abs(uz_gt) + np.abs(uz_pred) + 1e-9)) * 100)
    ...
    gs  = gridspec.GridSpec(1, 4, width_ratios=[1, 1, 1, 0.06],
                            wspace=0.08, left=0.05, right=0.93,
                            top=0.88, bottom=0.12)

    kw_img  = dict(cmap="viridis", vmin=vmin, vmax=vmax,
                   origin="lower", aspect="equal")
    kw_err  = dict(cmap="RdBu_r",
    ...
    ax0.imshow(uz_gt,   **kw_img);   ax0.set_title("Ground Truth $u_z$ (mm)", fontsize=10)
    im = ax1.imshow(uz_pred, **kw_img); ax1.set_title("CNN Prediction $u_z$ (mm)", fontsize=10)
    ax2.imshow(error,   **kw_err);   ax2.set_title("Error (Pred − GT, mm)", fontsize=10)
    ...
    fig.suptitle(
        f"ConvDecoderPredictor on Simulation {SIM_ID}  "
        f"|  MSE = {mse:.2e} mm²  |  sMAPE = {smape:.1f}%",
        fontsize=10, y=0.97
    )
...
def make_gui_composite():
    ...
    for i, (label, im) in enumerate(padded):
        ...
        ax2.text(xpos, 15, f"({chr(97+i)}) {label}",
