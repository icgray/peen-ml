...
def generate():
    sims = [d for d in os.listdir(OUT_DIR) if d.startswith("Simulation_")] \
...
def train():
    ...
    from model import (ConvDecoderPredictor, FieldDataset,
                       infer_grid_shape, infer_dataset_shape)
    ...
    n = len(dataset)
    n_tr = int(0.70 * n); n_va = int(0.15 * n); n_te = n - n_tr - n_va
    ...
    model = ConvDecoderPredictor(input_channels=1,
    ...
    for epoch in range(1, 121):
        ...
        if epoch % 10 == 0 or epoch <= 5:
            print(f"Epoch {epoch:3d}  train={tr_loss:.4e}  val={va_loss:.4e}  "
...
def evaluate_and_plot():
    ...
    rmse_fig  = float(np.sqrt(np.mean((pred_um[mask_fig]-gt_um[mask_fig])**2))) if mask_fig.sum()>0 else float("nan")
    ...
    gs  = gridspec.GridSpec(1, 5,
                            width_ratios=[0.7, 1, 1, 1, 0.06],
                            wspace=0.10, left=0.03, right=0.95,
    ...
    ax_cb.imshow(cb,        cmap="Blues",   **kw); ax_cb.set_title("Input\nCheckerboard", fontsize=9)
...
def make_gui_composite():
    ...
    if not imgs:
        print("No GUI images found."); return
...
def make_stl_clean():
    ...
    if not os.path.exists(src):
        print("STL_Deformation.png not found"); return
