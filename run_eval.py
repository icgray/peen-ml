...
MODEL_CATALOGUE = [
    ...
    (
        "A_DisplPredictor_Ti_Steel",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_Ti_6Al_4V__steel_200",
        "A - Std Ti+steel 200",
    ),
    (
        "B_ImprovedPredictor_Ti_Steel",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_Ti_6Al_4V__steel_200",
        "B - Improved Ti+steel 200",
    ),
    (
        "C_MatCond_MultiMat",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_MultiMat_Merged",
        "C - MatCond MultiMat 5000",
    ),
    (
        "D_Improved_Ti_6Al_4V__steel",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_Ti_6Al_4V__steel_200",
        "D - Improved Ti+steel 200",
    ),
    (
        "D_Improved_316L_SS__ceramic",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_316L_SS__ceramic_200",
        "D - Improved 316L+ceramic 200",
    ),
    (
        "D_Improved_Inconel_718__tungsten",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_Inconel_718__tungsten_200",
        "D - Improved Inconel+tungsten 200",
    ),
    (
        "D_Improved_Al_7075_T6__glass",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_Al_7075_T6__glass_200",
        "D - Improved Al+glass 200",
    ),
    (
        "D_Improved_4340_Steel__cast_iron",
        "trained_displacement_predictor_full_model.pth",
        "Dataset_4340_Steel__cast_iron_200",
        "D - Improved 4340+cast_iron 200",
    ),
    (
        "E_ConvDecoder_HighRes",
        "trained_conv_decoder_full_model.pth",
        "Dataset_HighRes_Ti_Steel_300",
        "E - ConvDecoder HighRes 300",
    ),
    ...
    (
        "MT_MultiTask_Ti_Steel",
        "trained_multitask_model.pth",
        "Dataset_Ti_6Al_4V__steel_200",
        "MT - MultiTask Ti+steel 200",
    ),
    (
        "I_InfluenceField_Ti_Steel",
        "influence_field_model.pth",
        "Dataset_Ti_6Al_4V__steel_200",
        "I - InfluenceField Ti+steel 200",
    ),
    (
        "I_InfluenceField_316L_ceramic",
        "influence_field_model.pth",
        "Dataset_316L_SS__ceramic_200",
        "I - InfluenceField 316L+ceramic 200",
    ),
    (
        "I_InfluenceField_Inconel_tungsten",
        "influence_field_model.pth",
        "Dataset_Inconel_718__tungsten_200",
        "I - InfluenceField Inconel+tungsten 200",
    ),
    (
        "I_InfluenceField_Al_glass",
        "influence_field_model.pth",
        "Dataset_Al_7075_T6__glass_200",
        "I - InfluenceField Al+glass 200",
    ),
    (
        "I_InfluenceField_4340_cast_iron",
        "influence_field_model.pth",
        "Dataset_4340_Steel__cast_iron_200",
        "I - InfluenceField 4340+cast_iron 200",
    ),
    # J: InfluenceField on 2000-sim (data-scaling ablation)
    (
        "J_InfluenceField_Ti_steel_2000",
        "influence_field_model.pth",
        "Dataset_Ti_6Al_4V__steel_2000",
        "J - InfluenceField Ti+steel 2000",
    ),
    # K: InfluenceField on HighRes (resolution-scaling ablation)
    (
        "K_InfluenceField_HighRes",
        "influence_field_model.pth",
        "Dataset_HighRes_Ti_Steel_300",
        "K - InfluenceField HighRes 300",
    ),
]
...
CROSS_EVALS = [
    ...
    (
        "G_Improved_Al7075_glass_2000",
        "model.pth",
        "Dataset_Ti_6Al_4V__steel_2000",
        "G - Improved Al+glass on Ti+steel (unseen)",
    ),
    (
        "H_Improved_Ti_steel_2000",
        "model.pth",
        "Dataset_Al_7075_T6__glass_2000",
        "H - Improved Ti+steel on Al+glass (unseen)",
    ),
]
...
def evaluate_multitask(
    ...
    cup_count = (
        sum(
            ...
            if d.startswith("Simulation_") and os.path.exists(os.path.join(dataset_dir, d, "cupping.npy"))
        )
...
def eval_model(
    ...
    row = {"label": label, "model_path": model_path, "dataset": os.path.basename(dataset_dir), "n_sims": n_sims}
...
def main():
    ...
    parser.add_argument("--run", default=DEFAULT_RUN, help="Run directory (default: LargeScaleRun1)")
    parser.add_argument("--models", nargs="*", help="Subset of model name prefixes to evaluate (e.g. G H MT)")
    parser.add_argument(
        "--components",
        nargs="*",
        default=["ux", "uy", "uz"],
    ...
    parser.add_argument("--no-cross", action="store_true", help="Skip cross-dataset evaluations")
    parser.add_argument(
        "--cupping", action="store_true", help="Run standalone cupping scatter for MT model and save PNG"
    )
    ...
    if args.models:
        catalogue = [e for e in catalogue if any(e[0].startswith(m) for m in args.models)]
    ...
    for model_dir_name, pth_file, dataset_rel, label in catalogue:
        ...
        if is_mt:
            row = evaluate_multitask(model_path, dataset_path, components, label, save_plot=plot_path)
        else:
            row = eval_model(model_path, dataset_path, components, label, save_plot=plot_path)
        rows.append(row)
        _flush_gpu()
    ...
    if not args.no_cross:
        ...
        if args.models:
            cross_catalogue = [e for e in cross_catalogue if any(e[0].startswith(m) for m in args.models)]
    ...
    if args.cupping:
        ...
        if os.path.exists(mt_model_path) and os.path.isdir(mt_dataset):
            ...
            try:
                ...
                print(f"  r={cup_res['pearson_r']:.4f}  RMSE={cup_res['rmse_um']:.3f} µm  " f"n={cup_res['n_ok']}")
            except Exception as exc:
