...
sims      = sorted([d for d in os.listdir(DATA) if d.startswith("Simulation_")],
...
def run(sn):
    ...
    pred = sample_field_at_coords(
        field, torch.tensor(nc[:, :2], dtype=torch.float32).to(device)
    )[0].cpu().numpy()
...
gt_flat = gt_2d.ravel(); pr_flat = pr_2d.ravel()
...
vmin = min(gt_2d.min(), pr_2d.min()); vmax = max(gt_2d.max(), pr_2d.max())
vcmin = min(gt_c.min(), pr_c.min());  vcmax = max(gt_c.max(), pr_c.max())

# ── 5 panels + colorbar ───────────────────────────────────────────────────────
fig = plt.figure(figsize=(16, 4.0))
gs  = gridspec.GridSpec(1, 6, width_ratios=[0.62, 1, 1, 1, 1, 0.06],
                        wspace=0.09, left=0.02, right=0.96,
                        top=0.87, bottom=0.10)

ax = [fig.add_subplot(gs[i]) for i in range(6)]
kw = dict(origin="lower", aspect="equal")

ax[0].imshow(cb,    cmap="Blues",   **kw); ax[0].set_title("Shot Density\nCheckerboard",            fontsize=9)
