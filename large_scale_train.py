...
def build_dataset_specs(n_sims_standard: int = 200,
                        n_sims_hires: int = 300,
                        workers: int = 4) -> Dict[str, List[DatasetSpec]]:
    ...
    for wp, sp, desc in key_combos:
        ...
        specs["per_material"].append(DatasetSpec(
            name=f"Dataset_{tag}_{n_sims_standard}",
            workpiece=wp, shot=sp,
    ...
    for wp in sorted(WORKPIECE_MATERIALS):
        for sp in sorted(SHOT_MATERIALS):
            tag = f"{wp.replace('-','_')}__{sp}"
            specs["multi_material"].append(DatasetSpec(
    ...
    specs["high_res"].append(DatasetSpec(
...
def generate_all_datasets(specs_dict: Dict[str, List[DatasetSpec]],
                          output_root: str,
                          skip_existing: bool = True) -> List[Dict]:
    """Generate all datasets sequentially (generation within each uses workers)."""
    all_specs = (
        specs_dict.get("per_material",   []) +
        specs_dict.get("multi_material", []) +
        specs_dict.get("high_res",       [])
    )
    results = []
    total = len(all_specs)
    ...
    for ci, spec in enumerate(all_specs, 1):
        ...
        if skip_existing and os.path.isdir(dataset_dir):
            ...
            if existing >= spec.n_sims:
                ...
                results.append({
                    "name": spec.name, "dataset_dir": dataset_dir,
                    "n_ok": existing, "n_fail": 0,
                    "elapsed_s": 0.0, "success": True, "error": None,
                })
                continue

        print(f"\n[{ci:3d}/{total}] Generating {spec.n_sims} sims — {spec.name}")
        print(f"          {spec.description}  "
              f"Nx={spec.Nx} Ny={spec.Ny} G={spec.G} workers={spec.workers}")
        res = generate_one_spec(spec, output_root)
        print(f"          Done: {res['n_ok']}/{spec.n_sims} OK  "
...
def merge_multi_material_datasets(source_dirs: List[str],
                                   merged_dir: str,
    ...
    existing_count = sum(
        1 for d in os.listdir(merged_dir)
        if d.startswith("Simulation_") and
        os.path.isdir(os.path.join(merged_dir, d))
    ...
    if skip_existing and existing_count > 0:
        ...
        for src in source_dirs:
            if os.path.isdir(src):
                total_available += sum(
                    1 for d in os.listdir(src)
                    if d.startswith("Simulation_") and
                    os.path.exists(os.path.join(src, d, "displacements.npy"))
                )
    ...
    try:
        ...
        if use_improved:
            model = M.ImprovedDisplacementPredictor(
                input_channels=1, num_nodes=num_nodes,
                checkerboard_size=cb_size, mat_dim=mat_dim,
            ).to(device)
        else:
        ...
        print(f"    {arch_name}: nodes={num_nodes} G={cb_size} "
        ...
        if ref_nc:
            shutil.copy2(str(ref_nc),
                         os.path.join(model_save_dir, "reference_node_coords.npy"))
...
def train_conv_decoder(
    ...
    try:
        ...
        n_params = sum(p.numel() for p in model.parameters())
        print(f"    ConvDecoderPredictor: grid={grid_H}×{grid_W}  G={G}  "
              f"mat_dim={mat_dim}  params={n_params:,}  device={device}")
...
def train_siren(
    ...
    try:
        train_loader, val_loader, test_loader, N_total, disp_scale = M.create_siren_loaders(
            dataset_dir, k_nodes=k_nodes, batch_size=batch_size,
            load_material_features=use_material,
        ...
        model = M.SIRENPredictor(
            input_channels=1, latent_dim=256, hidden=256,
            n_layers=4, mat_dim=mat_dim,
        ).to(device)

        n_params = sum(p.numel() for p in model.parameters())
        print(f"    SIRENPredictor: N_total={N_total} k_nodes={k_nodes}  "
              f"mat_dim={mat_dim}  params={n_params:,}  device={device}")
        ...
        for epoch in range(epochs):
            ...
            line1.set_xdata(range(1, len(train_losses)+1)); line1.set_ydata(train_losses)
            line2.set_xdata(range(1, len(val_losses)+1));   line2.set_ydata(val_losses)
            ax.relim(); ax.autoscale_view()
            print(f"    Epoch {epoch+1}/{epochs}  train={train_loss:.4e}  "
                  f"val={val_loss:.4e}")
        ...
        np.save(os.path.join(model_save_dir, "disp_scale.npy"),
                np.array([disp_scale], dtype=np.float64))
...
def print_summary(rows: List[Dict]) -> None:
    ...
    if ok:
        rmses = [float(r["rmse_um"]) for r in ok
                 if r.get("rmse_um") and str(r["rmse_um"]) != "nan"]
        if rmses:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="peen-ml large-scale training pipeline."
    )
    parser.add_argument("--output",   default="./LargeScaleResults",
                        help="Root output directory (default: ./LargeScaleResults)")
    parser.add_argument("--n_sims",   type=int, default=200,
                        help="Simulations per material combo (default 200)")
    parser.add_argument("--hires",    type=int, default=300,
                        help="Simulations for high-res ConvDecoder dataset (default 300)")
    ...
    parser.add_argument("--workers",  type=int, default=4,
                        help="Dataset generation workers per spec (default 4)")
    parser.add_argument("--phase",    default="all",
                        choices=["all", "generate", "train"],
                        help="Which phase to run (default: all)")
    parser.add_argument("--skip-gen", action="store_true",
                        help="Skip already-generated datasets")
    parser.add_argument("--quick",    action="store_true",
                        help="Quick mode: 20 sims, 15 epochs (for testing)")
    parser.add_argument("--tiny",     action="store_true",
                        help="Tiny mode: 10 sims, 5 epochs (smoke test)")
    parser.add_argument("--no-multimat", action="store_true",
                        help="Skip multi-material (all 25 combos) generation/training")
    parser.add_argument("--no-hires",    action="store_true",
                        help="Skip high-resolution ConvDecoder dataset/training")
    parser.add_argument("--no-siren",    action="store_true",
                        help="Skip SIREN model training")
    ...
    if args.phase in ("all", "train"):
        ...
        def run_training(model_name, train_fn, dataset_path, arch, n_sims, Nx, Ny,
                         batch_size_override=None, **kwargs):
            if not os.path.isdir(dataset_path):
                print(f"\n  [{model_name}] SKIP — dataset not found: {dataset_path}")
                report_rows.append({
                    "model_name": model_name, "dataset": dataset_path, "arch": arch,
                    "n_sims": n_sims, "Nx": Nx, "Ny": Ny,
            ...
            n_available = sum(
                ...
                if d.startswith("Simulation_") and
                os.path.exists(os.path.join(dataset_path, d, "displacements.npy"))
            )
            if n_available < 7:
                print(f"\n  [{model_name}] SKIP — only {n_available} sims (need ≥7)")
                report_rows.append({
                    "model_name": model_name, "dataset": dataset_path, "arch": arch,
                    "n_sims": n_available, "Nx": Nx, "Ny": Ny,
                    "success": False, "error": f"only {n_available} sims",
                })
                return

            model_save_dir = os.path.join(output_root, "Models", model_name)
            print(f"\n  [{model_name}]  arch={arch}  sims={n_available}  "
                  f"Nx={Nx} Ny={Ny}")
        ...
        run_training(
            ...
            n_sims     = args.n_sims, Nx=50, Ny=50,
            use_improved=False, use_material=False,
        )
        ...
        run_training(
            ...
            n_sims       = args.n_sims, Nx=50, Ny=50,
            use_improved = True, use_material=False,
        )
